...
Deterministic models assume the system follows a fixed and defined rule with no random variation or noise; stochastic models assume randomness or noise is present.
While deterministic paradigms assume the randomness is neglectable and consider only the average behavior of the system.
Stochastic models assume randomness which is not understandable or could not be implemented is important and explicitly include it in the system.
...
For the high or intermediate transmission settings, and systems that do not need to explore the heterogeneity, deterministic could be considered.
...
Deterministic models assume that known average rates with no random deviations are applied to large populations.
For example if 1,000 individuals each have a 90\% chance of surviving 1 year, then the model reasonably assume that 900 of them will indeed survive.


Traditionally, since the scale of malaria incidence globally, most models tended to be population-based, treating all individuals with compartment. Within a compartment individual were treated as identical, and 
...
MCMC has already been used in malaria ensemble modeling [108, 116, 118] and parameter estimation by Griffin et al[14], as well as in modeling of other infectious diseases [124, 125].
...
Further transparency includes sharing of the mathematics and code{Watson2017} of models. These small steps in documentation would allow for increased verification and validation of models, as well as increasing opportunities for collaboration between modeling groups.
